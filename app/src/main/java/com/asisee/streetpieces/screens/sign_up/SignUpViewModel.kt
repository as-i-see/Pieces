...
class SignUpViewModel @Inject constructor(
  ...
  fun onPhotoUriChosen(uri: Uri?) = uri?.let { chosenPhotoUri ->
    uiState.value = uiState.value.copy(photoUri = chosenPhotoUri)
  }

  fun onBioChange(newValue: String) {
    if (!newValue.isValidBio())
      SnackbarManager.showMessage(AppText.bio_error)
    else
  ...
  fun onSignUpClick(openAndPopUp: (String, String) -> Unit) {
    uiState.value.run {
      ...
      launchCatching {
        ...
        val remotePhotoUri = if (photoUri != Uri.EMPTY) photoStorageService.uploadAvatar(photoUri) else ""
        userDataStorageService.save(UserData(name = name, username = username, bio = bio, photoUri = remotePhotoUri))
        openAndPopUp(accountService.ownProfileRoute(), SIGN_UP_SCREEN)
      }
    }
  }
}
