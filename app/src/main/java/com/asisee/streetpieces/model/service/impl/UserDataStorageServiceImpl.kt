...
class UserDataStorageServiceImpl @Inject constructor(private val firestore: FirebaseFirestore, private val auth: AccountService):
    UserDataStorageService {
    override fun userData(userId: String): Flow<UserData> =
        firestore.collection(USERDATA_COLLECTION).whereEqualTo(USER_ID_FIELD, userId).dataObjects<UserData>().map {
            it.firstOrNull() ?: UserData()
        }

    override suspend fun getUserData(userId: String) = firestore.collection(USERDATA_COLLECTION).whereEqualTo(USER_ID_FIELD, userId).get().await().toObjects<UserData>().first()

    override suspend fun save(userData: UserData): String = trace(SAVE_USERDATA_TRACE) {
    ...
    override suspend fun update(userData: UserData): Unit = trace(UPDATE_USERDATA_TRACE) {
        firestore.collection(USERDATA_COLLECTION).document(userData.userId).set(userData).await()
    }
