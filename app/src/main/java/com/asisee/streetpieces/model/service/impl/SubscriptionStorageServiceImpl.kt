...
constructor(private val firestore: FirebaseFirestore, private val auth: AccountService) :
    ...
    override suspend fun subscribeCurrentUserTo(userId: String) =
        trace(SUBSCRIBE_TRACE) {
            firestore
                .collection(SUBSCRIPTIONS_COLLECTION)
                .add(Subscription(userId = auth.currentUserId, subUserId = userId))
                .await()
                .id
        }
    ...
    private fun subscriptionQuery(userId: String) =
        firestore
            .collection(SUBSCRIPTIONS_COLLECTION)
            .where(
                Filter.and(
    ...
    override suspend fun subscriptionId(userId: String) =
        subscriptionQuery(userId).get().await().documents.firstOrNull()?.id
